...
from text_search_comparison import compare as run_compare
...
def index():
    ...
    if request.method == "POST" and db_ok:
        form = {
            ...
            "search_mode": request.form.get("search_mode", DEFAULT_SEARCH_MODE),
        }

        # Validate search_mode value
        if form["search_mode"] not in SEARCH_MODES:
            form["search_mode"] = DEFAULT_SEARCH_MODE
...
def compare():
    ...
    form   = {"keyword": "batman", "collection": COLLECTION_SAMPLE, "runs": "1"}
    result = None
    error  = None

    if request.method == "POST" and db_ok:
        form = {
            "keyword":    request.form.get("keyword", "batman").strip(),
            "collection": request.form.get("collection", COLLECTION_SAMPLE),
            "runs":       request.form.get("runs", "1"),
        }
        keyword = form["keyword"]
        runs    = max(1, min(5, int(form["runs"]) if form["runs"].isdigit() else 1))

        if not keyword:
            error = "Please enter a keyword to compare."
        else:
            try:
                result = run_compare(keyword, form["collection"], runs=runs)
...
def chat():
    ...
    system_parts = [
        ...
        "If someone is unsure, ask one quick follow-up question to narrow it down. "
    ...
    try:
        ...
        return jsonify({"reply": "I needed too many database queries to answer that. Try a more specific question."})
